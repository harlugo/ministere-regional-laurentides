...
const RAYS = Array.from({length: 20}, (_, i) => {
  ...
  const r1 = isUp ? 160 + (i % 4 === 0 ? 30 : 12) : 290 + (i % 3 === 0 ? 45 : 12);
  return { x1: Math.cos(rad)*r0, y1: Math.sin(rad)*r0, x2: Math.cos(rad)*r1, y2: Math.sin(rad)*r1, thick: i%5===0, med: i%3===0 };
});

export default function WaveHero({ title, subtitle, tagline, ctaLabel, ctaHref }: WaveHeroProps) {
  return (
    <div className="relative overflow-hidden"
      style={{ background: "linear-gradient(175deg,#1a5090 0%,#3a8ec8 35%,#52a8d8 65%,#2a6035 100%)" }}>

      {/* ── BACKGROUND SCENE SVG — absolute, purely decorative ── */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        <svg viewBox="0 0 900 600" preserveAspectRatio="xMidYMax slice"
          ...
          <g transform="translate(595,288)">
            ...
            <rect x="22" y="96" width="6" height="54" rx="2" fill="#345a26"/><ellipse cx="25" cy="87" rx="22" ry="24" fill="#3e7038"/><ellipse cx="14" cy="97" rx="14" ry="16" fill="#346030"/>
            ...
            <polygon points="109,45 60,93 158,93" fill="#2a5230"/><polygon points="109,45 60,93 158,93" fill="none" stroke="#1e3a22" strokeWidth="1.4"/>
            <rect x="98" y="18" width="22" height="29" rx="2" fill="#f2efe6"/><rect x="98" y="18" width="22" height="29" rx="2" fill="none" stroke="#2a4828" strokeWidth="1.1"/>
            <polygon points="109,2 97,20 121,20" fill="#2a5230"/>
            ...
            <line x1="96" y1="142" x2="122" y2="142" stroke="#6a5030" strokeWidth="0.7"/>
      ...
      {/* ── CONTENT in normal flow: CROSS then TEXT ── */}
      <div className="relative z-10 flex flex-col items-center text-center px-6 pt-16 pb-24">

        {/* Cross SVG — fixed size, in flow, no absolute positioning */}
        <div style={{ width: "120px", height: "160px", position: "relative", marginBottom: "32px" }}>
          <svg viewBox="-160 -160 320 320" className="w-full h-full" overflow="visible">
            {/* Rays from centre (0,0) */}
            <g className="rays-breath">
              {RAYS.map((r, i) => (
                <line key={i} x1={r.x1} y1={r.y1} x2={r.x2} y2={r.y2}
                  stroke="white"
                  strokeWidth={r.thick ? 1.5 : r.med ? 0.9 : 0.5}
                  opacity={r.thick ? 0.88 : 0.55}/>
              ))}
            </g>
            {/* Halo */}
            <circle cx="0" cy="0" r="80" fill="white" fillOpacity="0.12" className="cross-glow"/>
            {/* Cross — vertical -62 to +68, horizontal at -18 */}
            <g className="cross-glow">
              <rect x="-7" y="-62" width="14" height="130" rx="4" fill="white" opacity="0.97"/>
              <rect x="-48" y="-18" width="96" height="14" rx="4" fill="white" opacity="0.97"/>
            </g>
          </svg>
        </div>
        ...
        <h1 className="fade-up-2 font-bold text-white leading-tight max-w-3xl"
          style={{ fontSize: "clamp(28px,5vw,54px)", textShadow: "0 2px 18px rgba(0,0,0,0.45)" }}>
          {title}
        </h1>
