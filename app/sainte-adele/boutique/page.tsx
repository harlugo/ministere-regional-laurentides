...
const PRODUCTS = [
  {
    ...
    desc: { fr: "Coupe unisexe en coton lavé premium. Message brodé au dos, logo chapelle devant. XS à 3XL.", en: "Unisex premium washed cotton. Embroidered back message, chapel logo front. XS to 3XL." },
    price: 52,
    colors: ["#DCC9A6","#2F4F3E","#888","#1a3a5c"],
    colorNames: { fr: ["Crème","Forêt","Gris","Marine"], en: ["Cream","Forest","Grey","Navy"] },
    tag: { fr: "Populaire ⭐", en: "Popular ⭐" },
    tagColor: "#E67E22",
  },
  {
    id: 2,
    img: "/boutique/casquette.jpg",
    collection: { fr: "Collection Laurentides", en: "Laurentians Collection" },
    name: { fr: "Casquette — Rosace", en: "Cap — Rose Window" },
    desc: { fr: "100% coton lavé. Broderie 3D rosace. Boucle ajustable en métal. Symbole d'unité et de lumière.", en: "100% washed cotton. 3D rose window embroidery. Metal adjustable buckle." },
    price: 28,
  ...
  {
    id: 4,
    img: "/boutique/tapis_entree.jpg",
    collection: { fr: "Collection Laurentides", en: "Laurentians Collection" },
    name: { fr: "Tapis d'entrée — Entre comme tu es", en: "Doormat — Come as you are" },
    desc: { fr: "Fibre de coco naturelle. Anti-dérapant, résistant. 60×90 cm. Intérieur et extérieur.", en: "Natural coconut fiber. Non-slip, durable. 60×90 cm. Indoor and outdoor." },
    price: 45,
    colors: ["#C9A96E"],
  ...
  {
    id: 5,
    img: "/boutique/rideau.jpg",
    collection: { fr: "Collection Lumière", en: "Light Collection" },
    name: { fr: "Rideau — Lumière & Paix", en: "Curtain — Light & Peace" },
    desc: { fr: "Inspiré des vitraux. Polyester de qualité. 140×240 cm. Œillets métalliques. Lavable à la machine.", en: "Inspired by stained glass. Quality polyester. 140×240 cm. Metal eyelets. Machine washable." },
    price: 65,
    colors: ["#DCC9A6","#7FA6C6","#C9A96E"],
    colorNames: { fr: ["Crème","Bleu","Sable"], en: ["Cream","Blue","Sand"] },
    tag: null, tagColor: "",
  },
  {
    id: 6,
    img: "/boutique/foulard.jpg",
    collection: { fr: "Collection Laurentides", en: "Laurentians Collection" },
    name: { fr: "Foulard — La lumière en nous", en: "Scarf — The light within us" },
    desc: { fr: "100% polyester. Léger et fluide. Impression recto, fini satiné. 70×70 cm.", en: "100% polyester. Light and fluid. Front print, satin finish. 70×70 cm." },
    price: 28,
    colors: ["#2F4F3E","#DCC9A6"],
    colorNames: { fr: ["Forêt","Crème"], en: ["Forest","Cream"] },
    tag: null, tagColor: "",
  },
  {
    id: 7,
    img: "/boutique/tapis_souris.jpg",
    collection: { fr: "Collection Lumière", en: "Light Collection" },
    name: { fr: "Tapis de souris — Paix. Lumière. Accueil.", en: "Mouse pad — Peace. Light. Welcome." },
    desc: { fr: "Surface polyester lisse. Base antidérapante. 23×19 cm. Résistant et durable.", en: "Smooth polyester surface. Non-slip rubber base. 23×19 cm. Durable." },
    price: 18,
...
const COLS_FR = ["Tout","Collection Lumière","Collection Laurentides"];
const COLS_EN = ["All","Light Collection","Laurentians Collection"];
...
export default function BoutiquePage() {
  ...
  const [selColors, setSelColors] = useState<Record<number,number>>({});
  const [added, setAdded] = useState<number|null>(null);

  const cols = lang === "fr" ? COLS_FR : COLS_EN;
  const filtered = filter === 0 ? PRODUCTS : PRODUCTS.filter(p => p.collection[lang] === cols[filter]);
  const cartTotal = cart.reduce((s,i) => s + i.price * i.qty, 0);
  const cartCount = cart.reduce((s,i) => s + i.qty, 0);

  const addToCart = (p: typeof PRODUCTS[0]) => {
    const ci = selColors[p.id] ?? 0;
    const color = p.colorNames[lang][ci];
    setCart(prev => {
      const ex = prev.find(i => i.id === p.id && i.color === color);
      if (ex) return prev.map(i => i.id===p.id && i.color===color ? {...i,qty:i.qty+1} : i);
      return [...prev, { id:p.id, name:p.name[lang], price:p.price, color, qty:1 }];
    });
    setAdded(p.id);
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#faf8f4" }}>
      ...
      {/* Hero */}
      <div className="py-16 px-6 text-center" style={{ background:"linear-gradient(135deg,#1a3d2b 0%,#2F4F3E 60%,#0d2440 100%)", paddingTop:"80px" }}>
        ...
        <p className="text-white/60 max-w-md mx-auto text-sm leading-relaxed mb-6" style={{ fontFamily:"system-ui" }}>
          {lang==="fr"
            ?"Des objets porteurs de sens — pour les membres, les visiteurs, et tous ceux qui cherchent un espace humain."
            :"Meaningful objects — for members, visitors, and everyone seeking a warm human space."}
        </p>
        <div className="flex h-1 max-w-xs mx-auto rounded-full overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-5 py-10">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="flex flex-wrap gap-2">
            {cols.map((c,i) => (
              <button key={i} onClick={() => setFilter(i)}
                ...
                style={{ fontFamily:"system-ui", background:filter===i?"#2F4F3E":"white", color:filter===i?"white":"#555", border:`1.5px solid ${filter===i?"#2F4F3E":"rgba(0,0,0,0.1)"}` }}>
                {c}
              </button>
            ))}
          </div>
          <button onClick={() => setCartOpen(true)}
            ...
            🛒 {lang==="fr"?"Panier":"Cart"}
            {cartCount > 0 && (
              <span className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold" style={{ background:"#E67E22", color:"white" }}>{cartCount}</span>
            )}
          </button>
        </div>
            ...
            return (
              <div key={p.id} className="card flex flex-col overflow-hidden">
                ...
                <div className="p-4 flex flex-col flex-1 gap-2">
                  ...
                  <div className="font-bold text-sm leading-snug" style={{ color:"#1a3d2b" }}>
                    {p.name[lang]}
                  </div>
                  ...
                  {p.colors.length > 1 && (
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-gray-400" style={{ fontFamily:"system-ui" }}>{p.colorNames[lang][ci]}</span>
                      <div className="flex gap-1.5">
                        {p.colors.map((c,i) => (
                          <button key={i} onClick={() => setSelColors(s => ({...s,[p.id]:i}))}
                            className="w-4 h-4 rounded-full transition-all hover:scale-110"
                            style={{ background:c, border:ci===i?"2px solid #1a3d2b":"2px solid transparent", boxShadow:ci===i?"0 0 0 1px #1a3d2b":"none" }}/>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between mt-1">
                    <span className="text-xl font-bold" style={{ color:"#2F4F3E", fontFamily:"system-ui" }}>{p.price} $</span>
                    <button onClick={() => addToCart(p)}
                      className="px-4 py-2 rounded-full text-xs font-bold transition-all hover:scale-105"
                      style={{ background:isAdded?"#27AE60":"#2F4F3E", color:"white", fontFamily:"system-ui", transition:"background 0.3s" }}>
                      {isAdded ? (lang==="fr"?"✓ Ajouté":"✓ Added") : (lang==="fr"?"Ajouter":"Add")}
                    </button>
                  </div>
                </div>
              </div>
        ...
        {/* Mission note */}
        <div className="mt-12 p-8 rounded-2xl text-center" style={{ background:"linear-gradient(135deg,#1a3d2b,#0d2440)" }}>
          <div className="text-2xl mb-3">🌿</div>
          <h3 className="text-xl font-bold text-white mb-2">
            {lang==="fr"?"Tu es chez toi ici.":"You are home here."}
          </h3>
          <p className="text-white/60 text-sm max-w-lg mx-auto leading-relaxed" style={{ fontFamily:"system-ui" }}>
            {lang==="fr"
              ?"100% des revenus soutiennent directement notre communauté — rénovation, jeunesse, aide alimentaire et concerts."
              :"100% of revenue directly supports our community — renovation, youth programs, food assistance, and concerts."}
          </p>
        </div>
      </div>
                    ...
                    <div key={i} className="flex items-start justify-between gap-3 p-3 rounded-xl" style={{ background:"#f8f8f5" }}>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button onClick={() => setCart(c => c.map(x => x===item&&x.qty>1?{...x,qty:x.qty-1}:x).filter(x=>x.qty>0))}
                          className="w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold" style={{ background:"#eee" }}>−</button>
            ...
            {cart.length > 0 && (
              <div className="p-5 border-t">
                <div className="flex justify-between mb-4">
                  <span className="font-bold" style={{ fontFamily:"system-ui" }}>Total</span>
                  <span className="text-xl font-bold" style={{ color:"#2F4F3E", fontFamily:"system-ui" }}>{cartTotal} $</span>
                </div>
                <button className="w-full py-3.5 rounded-full font-bold text-white text-sm"
