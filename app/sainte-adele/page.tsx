...
const T = {
  fr: {
    ...
    sched: [
      ...
      { e:"🎵", day:"Mensuel", time:"19h30", label:"Concert Lambert — 15-25$" },
    ],
    services: [
    ...
    contactSub: "Pour tout renseignement sur les mariages, baptêmes ou locations",
    contactName: "Jacques-Henri Honoré",
    contactRole: "Responsable des locations",
    contactPhone: "(450) 512-8007",
    steph: "Stéphane Godbout",
    stephRole: "Pasteur",
    stephBio: "Stéphane anime notre communauté avec chaleur et profondeur depuis plusieurs années. Ses réflexions touchent le cœur de chacun chaque dimanche matin.",
    lambert: "Lambert",
    lambertRole: "Musicien en résidence",
  ...
  en: {
    ...
    sched: [
      ...
      { e:"🎵", day:"Monthly", time:"7:30 PM", label:"Lambert concert — $15-25" },
    ],
    services: [
    ...
    contactSub: "For all inquiries about weddings, baptisms or rentals",
    contactName: "Jacques-Henri Honoré",
    contactRole: "Rentals coordinator",
    contactPhone: "(450) 512-8007",
    steph: "Stéphane Godbout",
    stephRole: "Pastor",
    stephBio: "Stéphane brings warmth and depth to our community every week. His thoughtful reflections touch everyone's heart each Sunday morning.",
    lambert: "Lambert",
    lambertRole: "Musician in residence",
  ...
  return (
    <div>
      ...
      <section id="horaire" className="py-20 px-6 md:px-12 max-w-5xl mx-auto">
        <SectionTitle title={lang==="fr"?"Horaire & Activités":"Schedule & Activities"} sub={lang==="fr"?"Tous les rendez-vous réguliers de notre communauté":"All regular community gatherings"} color="#2980B9"/>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-10">
          {t.sched.map((s,i) => (
            <div key={i} className="card p-6 text-center">
              ...
              <div className="text-2xl font-bold mb-2" style={{color:WAVES[i % WAVES.length], fontFamily:"system-ui"}}>{s.time}</div>
      ...
      <section id="services" className="py-20 px-6 md:px-12" style={{background:"#f0f4f0"}}>
        <div className="max-w-6xl mx-auto">
          <SectionTitle title={lang==="fr"?"Services & Activités":"Services & Activities"} sub={lang==="fr"?"Tout ce que notre communauté offre":"Everything our community offers"} color="#27AE60"/>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-10">
            {t.services.map((s,i) => (
              <div key={i} className="card p-5 bg-white">
                <div className="text-2xl mb-3">{s.i}</div>
                <div className="font-bold text-base mb-2" style={{color:"#1a3d2b", fontFamily:"system-ui"}}>{s.t}</div>
                <p className="text-gray-500 text-sm leading-relaxed" style={{fontFamily:"system-ui"}}>{s.b}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-10 grid md:grid-cols-2 gap-8">
          <div className="card p-8">
            <div className="flex items-center gap-4 mb-5">
              ...
              <div>
                <div className="font-bold text-lg" style={{color:"#1a3d2b", fontFamily:"system-ui"}}>{t.lambert}</div>
                <div className="text-sm text-gray-500" style={{fontFamily:"system-ui"}}>{t.lambertRole}</div>
              </div>
            </div>
          ...
          <div className="card p-8">
            <div className="flex items-center gap-4 mb-5">
              ...
              <div>
                <div className="font-bold text-lg" style={{color:"#1a3d2b", fontFamily:"system-ui"}}>{t.steph}</div>
                <div className="text-sm text-gray-500" style={{fontFamily:"system-ui"}}>{t.stephRole}</div>
              </div>
            </div>
      ...
      <section id="don" className="py-20 px-6 text-center" style={{background:"linear-gradient(135deg,#0d2440,#1a3d2b)"}}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mt-8">
          <a href="#" className="px-8 py-4 rounded-full font-bold text-sm transition-all hover:scale-105 text-white" style={{background:"#27AE60", fontFamily:"system-ui"}}>
            {t.donRecur} →
          </a>
          <a href="#" className="px-8 py-4 rounded-full font-bold text-sm transition-all hover:scale-105 text-white" style={{background:"rgba(255,255,255,0.12)", border:"1.5px solid rgba(255,255,255,0.25)", fontFamily:"system-ui"}}>
            {t.donOnce} →
          </a>
        </div>
      </section>
      ...
      <section id="contact" className="py-20 px-6 md:px-12 max-w-3xl mx-auto">
        ...
        <div className="mt-10 grid sm:grid-cols-2 gap-6">
          <div className="card p-6 text-center">
            <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-3 text-xl font-bold" style={{color:"#1a3d2b"}}>JH</div>
            <div className="font-bold" style={{color:"#1a3d2b", fontFamily:"system-ui"}}>{t.contactName}</div>
            <div className="text-xs text-gray-500 mt-0.5 mb-3" style={{fontFamily:"system-ui"}}>{t.contactRole}</div>
            <a href="tel:4505128007" className="text-lg font-bold" style={{color:"#2980B9", fontFamily:"system-ui"}}>{t.contactPhone}</a>
          </div>
          <div className="card p-6 text-center">
            ...
            <div className="font-bold" style={{color:"#1a3d2b", fontFamily:"system-ui"}}>{t.steph}</div>
            <div className="text-xs text-gray-500 mt-0.5 mb-3" style={{fontFamily:"system-ui"}}>{t.stephRole}</div>
            <div className="flex flex-col gap-1">
              {["⛪ Dimanche 10h30","📍 Sainte-Adèle, QC"].map((l,i)=>(
                <div key={i} className="text-sm text-gray-500" style={{fontFamily:"system-ui"}}>{l}</div>
              ))}
            </div>
          </div>
        </div>
